...
exports.core = async (sock, mei) => {
	...
	case `stiker`:
	{
		...
		if (!img && !gif)return g.func.reply("Format Salah!")
		...
		sock.sendMessage(from, await sticker.toMessage())
...
const template = generateWAMessageFromContent(from, proto.Message.fromObject({
                            templateMessage: {
                                hydratedTemplate: {
									...
									hydratedFooterText: `Hai, Ada yang bisa Riri Bantu?
_Prefik:_ ( . )

LIST MENU :                      
➸ .sticker`,
									locationMessage: { jpegThumbnail: fs.readFileSync('./assets/menu.jpg') },
                                    hydratedButtons: [{
                                        urlButton: {
                                            displayText: 'Owner',
